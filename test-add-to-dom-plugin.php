...
function polymuse_add_model_and_thumbnail_to_gallery($html, $attachment_id)
{
    ...
    if (!empty($model_url)) {
        ...
        if ($first_image) {
            ...
            $model_viewer .= '<model-viewer src="' . esc_url($model_url) . '" alt="3D model of ' . esc_attr($product->get_name()) . '" auto-rotate camera-controls ar ar-modes="webxr"ar-scale="fixed" style="width: 100%; height: 100%;"></model-viewer>';
            $model_viewer .= '</div>';
